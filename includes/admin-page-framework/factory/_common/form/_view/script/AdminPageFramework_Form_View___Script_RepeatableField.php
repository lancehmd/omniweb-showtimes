...
class OmniwebShowtimes_AdminPageFramework_Form_View___Script_RepeatableField extends OmniwebShowtimes_AdminPageFramework_Form_View___Script_Base {
...
(function ( $ ) {
     ...
     */
    $.fn.updateOmniwebShowtimes_AdminPageFrameworkRepeatableFields = function( aSettings ) {
        ...
        if ( ! $.fn.aOmniwebShowtimes_AdminPageFrameworkRepeatableFieldsOptions ) {
            $.fn.aOmniwebShowtimes_AdminPageFrameworkRepeatableFieldsOptions = [];
        }
        if ( ! $.fn.aOmniwebShowtimes_AdminPageFrameworkRepeatableFieldsOptions.hasOwnProperty( _sFieldsContainerID ) ) {     
            $.fn.aOmniwebShowtimes_AdminPageFrameworkRepeatableFieldsOptions[ _sFieldsContainerID ] = $.extend({    
        ...
        var _aOptions = $.fn.aOmniwebShowtimes_AdminPageFrameworkRepeatableFieldsOptions[ _sFieldsContainerID ];
...
// @todo For nested fields, the `find()` method should be avoided.
        $( nodeThis ).find( '.omniweb-showtimes-repeatable-field-buttons' ).attr( 'data-max', _aOptions[ 'max' ] );
        $( nodeThis ).find( '.omniweb-showtimes-repeatable-field-buttons' ).attr( 'data-min', _aOptions[ 'min' ] );
        $( nodeThis ).find( '.omniweb-showtimes-repeatable-field-buttons' ).attr( 'data-fadein', _aOptions[ 'fadein' ] );
        $( nodeThis ).find( '.omniweb-showtimes-repeatable-field-buttons' ).attr( 'data-fadeout', _aOptions[ 'fadeout' ] );
...
// @todo For nested fields, the `find()` method should be avoided.         
        ...
        $( nodeThis ).find( '.repeatable-field-add-button' ).click( function() {
            $( this ).addOmniwebShowtimes_AdminPageFrameworkRepeatableField();
...
// @todo For nested fields, the `find()` method should be avoided.        
        ...
        $( nodeThis ).find( '.repeatable-field-remove-button' ).click( function() {
            $( this ).removeOmniwebShowtimes_AdminPageFrameworkRepeatableField();
            return false; // will not click after that
        });

        /* If the number of fields is less than the set minimum value, add fields. */
        var _sFieldID           = nodeThis.find( '.repeatable-field-add-button' ).first().closest( '.omniweb-showtimes-field' ).attr( 'id' );
        var _nCurrentFieldCount = jQuery( '#' + _sFieldsContainerID ).find( '.omniweb-showtimes-field' ).length;
        if ( _aOptions[ 'min' ] > 0 && _nCurrentFieldCount > 0 ) {
            if ( ( _aOptions[ 'min' ] - _nCurrentFieldCount ) > 0 ) {     
                $( '#' + _sFieldID ).addOmniwebShowtimes_AdminPageFrameworkRepeatableField( _sFieldID );  
            }
        }
     ...
     */
    $.fn.addOmniwebShowtimes_AdminPageFrameworkRepeatableField = function( sFieldContainerID ) {
        
        if ( 'undefined' === typeof sFieldContainerID ) {
            var sFieldContainerID = $( this ).closest( '.omniweb-showtimes-field' ).attr( 'id' );
        }

        var nodeFieldContainer  = $( '#' + sFieldContainerID );
        var nodeNewField        = nodeFieldContainer.clone(); // clone without bind events.
        var nodeFieldsContainer = nodeFieldContainer.closest( '.omniweb-showtimes-fields' );
        var _sFieldsContainerID = nodeFieldsContainer.attr( 'id' );

        // If the set maximum number of fields already exists, do not add.
        if ( ! $.fn.aOmniwebShowtimes_AdminPageFrameworkRepeatableFieldsOptions.hasOwnProperty( _sFieldsContainerID ) ) {     
            var nodeButtonContainer = nodeFieldContainer.find( '.omniweb-showtimes-repeatable-field-buttons' );
            $.fn.aOmniwebShowtimes_AdminPageFrameworkRepeatableFieldsOptions[ _sFieldsContainerID ] = {    
        ...
        var _iFadein  = $.fn.aOmniwebShowtimes_AdminPageFrameworkRepeatableFieldsOptions[ _sFieldsContainerID ][ 'fadein' ];
        var _iFadeout = $.fn.aOmniwebShowtimes_AdminPageFrameworkRepeatableFieldsOptions[ _sFieldsContainerID ][ 'fadeout' ];

        // Show a warning message if the user tries to add more fields than the number of allowed fields.
        var sMaxNumberOfFields = $.fn.aOmniwebShowtimes_AdminPageFrameworkRepeatableFieldsOptions[ _sFieldsContainerID ]['max'];
        if ( sMaxNumberOfFields != 0 && nodeFieldsContainer.find( '.omniweb-showtimes-field' ).length >= sMaxNumberOfFields ) {
            var nodeLastRepeaterButtons = nodeFieldContainer.find( '.omniweb-showtimes-repeatable-field-buttons' ).last();
         ...
         */
        nodeNewField.updateOmniwebShowtimes_AdminPageFrameworkRepeatableFields();
        ...
        // For nested fields,
/*         $( nodeNewField ).find( '.omniweb-showtimes-field' ).each( function( iIterationIndex ) {    
...
// @todo find() may not be appropriate for nested fields.
        ...
        nodeFieldsContainer.children( '.omniweb-showtimes-field' ).children( '.delimiter' ).show().last().hide();
        ...
        var _incrementFieldAttributes = function( oElement, oFieldsContainer ) {
            ...
            oElement.find( '.omniweb-showtimes-field,.omniweb-showtimes-fields,.omniweb-showtimes-fieldset' ).incrementAttributes(
...
// console.log( oElement.find( 'fieldset.omniweb-showtimes-fieldset' ).length );
// console.log( oElement.find( 'fieldset.omniweb-showtimes-fieldset' ).first().attr( 'id' ) );
            // oElement.find( 'fieldset.omniweb-showtimes-fieldset' ).incrementAttributes(
...
// console.log( oElement.find( 'fieldset.omniweb-showtimes-fieldset' ).first().attr( 'id' ) );
     ...
     * Removes a repeatable field.
      This method is called when the user presses the - repeatable button.
     */
    $.fn.removeOmniwebShowtimes_AdminPageFrameworkRepeatableField = function() {
        
        /* Need to remove the element: the field container */
        var nodeFieldContainer  = $( this ).closest( '.omniweb-showtimes-field' );
        var nodeFieldsContainer = $( this ).closest( '.omniweb-showtimes-fields' );
        var _sFieldsContainerID = nodeFieldsContainer.attr( 'id' );
        
        /* If the set minimum number of fields already exists, do not remove */
        var sMinNumberOfFields  = $.fn.aOmniwebShowtimes_AdminPageFrameworkRepeatableFieldsOptions[ _sFieldsContainerID ][ 'min' ];
        if ( sMinNumberOfFields != 0 && nodeFieldsContainer.find( '.omniweb-showtimes-field' ).length <= sMinNumberOfFields ) {
            var nodeLastRepeaterButtons = nodeFieldContainer.find( '.omniweb-showtimes-repeatable-field-buttons' ).last();
            ...
            var _iFadeout = $.fn.aOmniwebShowtimes_AdminPageFrameworkRepeatableFieldsOptions[ _sFieldsContainerID ][ 'fadeout' ]
                ? $.fn.aOmniwebShowtimes_AdminPageFrameworkRepeatableFieldsOptions[ _sFieldsContainerID ][ 'fadeout' ]
                : 500;
            nodeMessage.delay( 2000 ).fadeOut( _iFadeout );
            return;     
        }     
